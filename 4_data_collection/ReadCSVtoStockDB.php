...
//This module will read from a CSV file and save the data into the Tickers Table.
$servername = "localhost";
$username = "root";
$password = "";
$dbname = "StockDBPDO";
...
if (($handle = fopen("$path/$id.csv", "r")) !== FALSE) {
    echo "<p> Reading data from $id.csv<br /></p>\n";
    echo "<p> Updating Table Tickers1...<br /></p>\n";
    while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
        $num = count($data);
        $row++;
    ...
    // set the PDO error mode to exception
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "INSERT INTO Tickers (tickerid, stockname)
